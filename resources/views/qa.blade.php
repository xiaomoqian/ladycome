...
<html lang="zh-CN">
  <head>
    ...
    @foreach($seo as $key => $value)
      @if($value['page_type'] == 3)
        ...
        <title>{{$value['seo_title']??"问答"}}</title>
        @break
      @endif
    @endforeach
  ...
  <body>
    ...
    <div class="qa-navbar">
      <div class="container">
        <div>
          <a href="{{route('qa')}}">问答首页</a>
          @foreach($label as $key => $value)
            <a href="{{route('wfenlei',['type' => $value['id']])}}">{{$value['name']}}</a>
          @endforeach
        </div>
      </div>
    </div>
      ...
      <div class="container-news">
        <div class="row">
          <div class="container-news__left col-lg-9 col-md-9 col-sm-12 col-xs-12">
            <div class="nav">当前位置：<a href="{{route('index')}}">多广网</a>><a href="{{route('qa')}}">问答广场</a><span>>{{$lable_name??""}}</span></div>
            <img src="./image/qa.png" width="100%" alt="">
            <ul class="container-news__left--box">
              @foreach($qa as $key => $value)
                <li>
                  <a href="{{route('wdinfo',['id' => $value['id']])}}">
                    
                    <img alt="{{$value['title']}}" src="{{$value['photo']!= ""?strstr($value['photo'],'http')?$value['photo']:asset("/storage/".$value['photo']):asset("/storage/images/Koala.jpg")}}">
          ...
          <div class="author">
              ...
              <ul>
                @foreach($label as $key => $value)
                  <li>
                    <a href="{{route('wfenlei',['type' => $value['id']])}}">
                      {{$value['name']}}
                    </a>
                  </li>
                @endforeach
