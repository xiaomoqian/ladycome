...
<html lang="zh-CN">
  <head>
    ...
    <meta name="description" content="{{$seience['seo_desc']??''}}">  
    <meta name="keywords" content="{{$seience['seo_keywords']??''}}">
    <!-- 上述3个meta标签*必须*放在最前面，任何其他内容都*必须*跟随其后！ -->
    <title>{{$seience['title']??""}} - 多广</title>
        ...
        <div class="row">
          <div class="container-news__left col-lg-9 col-md-9 col-sm-12 col-xs-12">
            <div class="container-news__left--box">
              <div class="nav">当前位置：<a href="{{route('index')}}">多广网</a>><a href="{{route('knowledge')}}">知识</a>><a href="#">{{$seience['title']??""}}</a></div>
              <h3>{{$seience['title']??""}} </h3>
              <p class="author">
                <a href="{{route('author',['id' => $seience['author_id']])}}">{{$seience['author_name']??""}} </a>
                {{$seience['created_at']??""}}
              </p>
              <div class="desc">摘要：{{$seience['seo_desc']??""}}</div>
              <div class="content">
                {!! $seience['contont']??"" !!}
              </div>
              <div class="tags">
                @foreach($seience['label'] as $key => $value)
                  <span><a href="{{route('fenlei',['type' => $value['id']])}}">{{$value['name']}}</a></span>
                @endforeach
              </div>
              <div class="ad">
              ...
              <div class="tuijian">
                ...
                <ul>
                  @foreach($relevant as $key => $value)
                    <li><a href="{{route('zsinfo',['id' => $value['id']])}}">{{$value['title']??""}}</a></li>
                  @endforeach
                </ul>
              </div>
              <div class="mad">
          ...
          <div class="container-news__right hidden-xs col-lg-3 col-md-3 hidden-sm">
            <div class="question">
              <h4>最新知识</h4>
              <ul>
                @foreach($xin_seience as $key => $value)
                  <li>
